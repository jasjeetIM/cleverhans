...
class CarliniWagnerL2(object):
    ...
    def __init__(self, sess, model, batch_size, confidence,
        ...
        if self.use_cos_norm_reg:
          self.gimg = tf.Variable(np.zeros(shape), dtype=tf.float32,
                                name='gimg')
        ...
        if self.use_cos_norm_reg:
          self.output_guide = model.get_logits(self.gimg)

        #Get cross entropy loss for the model
        self.loss_curr = tf.nn.softmax_cross_entropy_with_logits(logits=self.output, labels=self.tlab)
        if self.use_cos_norm_reg:
